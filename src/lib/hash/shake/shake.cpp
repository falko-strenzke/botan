...
void SHAKE_128::final_result(uint8_t output[])
   ...
   Keccak_FIPS_generic::finish(SHAKE_128_BITRATE, m_S, m_S_pos, 0xF, 4);
   Keccak_FIPS_generic::expand(SHAKE_128_BITRATE, m_S, output, output_length());
   clear();
   }
...
void SHAKE_256::add_data(const uint8_t input[], size_t length)
   {
   m_S_pos = Keccak_FIPS_generic::absorb(SHAKE_256_BITRATE, m_S, m_S_pos, input, length);
   }

void SHAKE_256::final_result(uint8_t output[])
   {
   Keccak_FIPS_generic::finish(SHAKE_256_BITRATE, m_S, m_S_pos, 0xF, 4);
   Keccak_FIPS_generic::expand(SHAKE_256_BITRATE, m_S, output, output_length());

...
class Dilithium_Symmetric_Primitives
   ...
   public:
      static std::unique_ptr<Dilithium_Symmetric_Primitives> create(DilithiumFlavor algo);

      virtual ~Dilithium_Symmetric_Primitives() = default;

      // H is same for all modes
      template <typename Alloc>
      secure_vector<uint8_t> H(const std::vector<uint8_t, Alloc>& seed, size_t out_len) const
         {
         return SHAKE_256(out_len * 8).process(seed);
         }

      // CRH is same for all modes
      secure_vector<uint8_t> CRH(const uint8_t* in, size_t in_len,
                                 size_t out_len) const
         {
         return SHAKE_256(out_len * 8).process(in, in_len);
         }

      // Mode dependent function
      virtual std::unique_ptr<StreamCipher> XOF(const XofType type, const std::vector<uint8_t>& seed,
            uint16_t matrix_position) const = 0;

      virtual secure_vector<uint8_t> ExpandMask(const secure_vector<uint8_t>& seed,
            uint16_t nonce, size_t out_len) const = 0;
...
class DilithiumModeConstants
   ...
   public:
      ...
      DilithiumModeConstants(DilithiumDimension dimension, DilithiumFlavor algo);

      DilithiumModeConstants(const DilithiumModeConstants& other)
         : DilithiumModeConstants(other.m_dimension, other.m_algo) {}
      ...
      OID oid() const
         {
         std::vector<uint32_t> result{1,3,6,1,4,1,25258,1};
         switch(m_algo)
            {
         ...
         uint32_t param_value = static_cast<uint32_t>(m_dimension);
         switch(m_algo)
            {
            case DilithiumFlavor::Deterministic:
            case DilithiumFlavor::AesDeterministic:
               result.push_back(param_value * 2);
               break;
            case DilithiumFlavor::Randomized:
            case DilithiumFlavor::AesRandomized:
               result.push_back(param_value * 2 - 1);
               break;
            }
         return OID(std::move(result));
         }
      ...
      bool use_randomized() const
         {
         return m_use_randomize;
         }
      ...
      // Wrapper
      template <typename Alloc>
      decltype(auto) H(const std::vector<uint8_t, Alloc>& seed, size_t out_len) const
         {
         return m_symmetric_primitives->H(seed, out_len);
         }

      secure_vector<uint8_t> CRH(const secure_vector<uint8_t>& in) const
         {
         return m_symmetric_primitives->CRH(in.data(), in.size(), DilithiumModeConstants::CRHBYTES);
         }
      secure_vector<uint8_t> CRH(const std::vector<uint8_t>& in) const
         {
         return m_symmetric_primitives->CRH(in.data(), in.size(), DilithiumModeConstants::CRHBYTES);
         }

      std::unique_ptr<StreamCipher> XOF_128(const std::vector<uint8_t>& seed,
                                            uint16_t nonce) const
         {
         return this->m_symmetric_primitives->XOF(Dilithium_Symmetric_Primitives::XofType::k128, seed, nonce);
         }
      std::unique_ptr<StreamCipher> XOF_256(const std::vector<uint8_t>& seed,
                                            uint16_t nonce) const
         {
   ...
   private:
      DilithiumDimension m_dimension;
      DilithiumFlavor m_algo;

      // use randomization at signature creation
      bool m_use_randomize = false;
      bool m_use_aes = false;
      uint8_t m_nist_security_strength;

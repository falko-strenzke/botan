...
class Dilithium_Signature_Operation final : public PK_Ops::Signature
   {
   public:
      Dilithium_Signature_Operation(const Dilithium_PrivateKey& priv_key_dilithium) :
      ...
      secure_vector<uint8_t> sign(RandomNumberGenerator& rng) override
         ...
         const auto rhoprime = (mode.use_randomized())
...
void ensure_dilithium_alg_id(const AlgorithmIdentifier& alg_id)
   {
   const auto oid_str = alg_id.oid().to_formatted_string();
   const auto algo = split_on(oid_str, '/').front();
   if(algo != "Dilithium-r3" && algo != "Dilithium-AES-r3")
      {
      throw Lookup_Error("Provided algorithm identifier is not Dilithium");
      }
   }

DilithiumDimension dimension_from_alg_id(const AlgorithmIdentifier& alg_id)
   {
   ensure_dilithium_alg_id(alg_id);
   switch(alg_id.oid().get_components().back())
      {
      case 1:
      case 2:
        return DilithiumDimension::Dilithium4x4;
      case 3:
      case 4:
        return DilithiumDimension::Dilithium6x5;
      case 5:
      case 6:
        return DilithiumDimension::Dilithium8x7;
      }

   throw Lookup_Error("Unsupported Dilithium dimension");
   }

DilithiumFlavor algo_from_alg_id(const AlgorithmIdentifier& alg_id)
   {
   ensure_dilithium_alg_id(alg_id);
   const bool aes = alg_id.oid().get_components().at(alg_id.oid().get_components().size() - 2) == 10;
   switch(alg_id.oid().get_components().back())
      {
      case 1:
      case 3:
      case 5:
         return aes ? DilithiumFlavor::AesRandomized : DilithiumFlavor::Randomized;
      case 2:
      case 4:
      case 6:
         return aes ? DilithiumFlavor::AesDeterministic : DilithiumFlavor::Deterministic;
      }

   throw Lookup_Error("Unsupported Dilithium flavor");
   }

}

Dilithium_PublicKey::Dilithium_PublicKey(const AlgorithmIdentifier& alg_id, const std::vector<uint8_t>& pk)
   : Dilithium_PublicKey(pk, dimension_from_alg_id(alg_id), algo_from_alg_id(alg_id), DilithiumKeyEncoding::Raw) {}

Dilithium_PublicKey::Dilithium_PublicKey(const std::vector<uint8_t>& pk, DilithiumDimension dimension,
      DilithiumFlavor algo, DilithiumKeyEncoding encoding) : m_key_encoding(encoding)
   {
   DilithiumModeConstants mode(dimension, algo);
...
std::unique_ptr<PK_Ops::Verification> Dilithium_PublicKey::create_verification_op(const std::string& params,
   ...
   {
   BOTAN_UNUSED(params, provider);
   return std::unique_ptr<PK_Ops::Verification>(new Dilithium_Verification_Operation(*this));
   }

Dilithium_PrivateKey::Dilithium_PrivateKey(RandomNumberGenerator& rng, DilithiumDimension dimension, DilithiumFlavor algo)
   {
   DilithiumModeConstants mode(dimension, algo);
...
Dilithium_PrivateKey::Dilithium_PrivateKey(const AlgorithmIdentifier& alg_id, const secure_vector<uint8_t>& sk)
   : Dilithium_PrivateKey(sk, dimension_from_alg_id(alg_id), algo_from_alg_id(alg_id), DilithiumKeyEncoding::Raw) {}
...
Dilithium_PrivateKey::create_signature_op(RandomNumberGenerator& rng,
   ...
   return std::unique_ptr<PK_Ops::Signature>(new Dilithium_Signature_Operation(*this));
   }

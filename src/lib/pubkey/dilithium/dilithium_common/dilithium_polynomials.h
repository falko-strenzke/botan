   ...
   public:
      ...
      **************************************************/
      static void fill_poly_uniform_eta(Polynomial& a, const secure_vector<uint8_t>& seed,
         ...
         // TODO: We're deliberately unlocking a secure_vector (to fulfil the
         //       XOF API). That might be a problem: double-check whether seed
         //       needs to secure.
         const std::vector<uint8_t> seed_vec = unlock(seed);
         auto cipher = mode.XOF_256(seed_vec, nonce);

         secure_vector<uint8_t> buf;
         buf.resize(mode.poly_uniform_eta_nblocks() * mode.stream256_blockbytes());
         ...
         while(ctr < DilithiumModeConstants::N)
            ...
            zero_input.resize(mode.stream256_blockbytes());
            cipher->cipher(zero_input.data(), buf.data(), mode.stream256_blockbytes());
...
class PolynomialVector
   ...
   public:
      // Default Consturctor inserted. Check further features for this
      ...
      **************************************************/
      static Polynomial poly_uniform(const std::vector<uint8_t>& seed,
         ...
         std::vector<uint8_t> buf(mode.poly_uniform_nblocks()* mode.stream128_blockbytes() + 2);

         auto cipher = mode.XOF_128(seed, nonce);
         cipher->cipher1(buf.data(), mode.poly_uniform_nblocks()*mode.stream128_blockbytes());
         ...
         while(ctr < DilithiumModeConstants::N)
            ...
            std::vector<uint8_t> buf2(mode.stream128_blockbytes());
            cipher->cipher(buf2.data(), buf.data() + off, mode.stream128_blockbytes());

...
std::string KMAC256::name() const { return std::string("KMAC256(" + std::to_string(m_output_bit_length) + ")"); }

std::unique_ptr<MessageAuthenticationCode> KMAC256::new_object() const {
   return std::make_unique<KMAC256>(m_output_bit_length);
}

size_t KMAC256::output_length() const { return m_output_bit_length / 8; }
...
bool KMAC256::has_keying_material() const { return m_key_set; }
...
void KMAC256::add_data(const unsigned char* data, unsigned long data_len) {
   assert_key_material_set(m_key_set);

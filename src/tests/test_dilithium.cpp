...
namespace
{
   ...
   Test::Result run_dilithium_kat_test(const char* test_name, const VarMap& vars, Botan::DilithiumDimension dimension, Botan::DilithiumFlavor algo,
      [[maybe_unused]] const std::string& algo_name)
   {
      ...
      Botan::Dilithium_PrivateKey priv_key(*dilithium_test_rng, dimension, algo);
      ...
      auto signer = Botan::PK_Signer(priv_key, *dilithium_test_rng, "");
      ...
      Botan::Dilithium_PublicKey pub_key(priv_key.public_key_bits(), dimension, algo, Botan::DilithiumKeyEncoding::Raw);
      auto verificator = Botan::PK_Verifier(pub_key,"");
      verificator.update(ref_msg.data(), ref_msg.size());
      ret_val = verificator.check_signature(signature.data(), signature.size());
      result.test_is_eq("Signature Verification: ", ret_val, 1);

      // wrong signagture
      auto mutated_signature = Test::mutate_vec(signature);
      ret_val = verificator.check_signature(mutated_signature.data(), mutated_signature.size());
      result.test_is_eq("Wrong Signature Verification: ", ret_val, 0);

      return result;
   }
}

#define REGISTER_DILITHIUM_KAT_TEST(mode, algo)                                                                                             \
class DILITHIUM##mode##algo final : public Text_Based_Test                                                                                  \
   {                                                                                                                                        \
   public:                                                                                                                                  \
      DILITHIUM##mode##algo() : Text_Based_Test("pubkey/dilithium_" #mode "_" #algo ".vec", "count,seed,msg,pk_sha3_256,sk_sha3_256,sm") {} \
      Test::Result run_one_test(const std::string &name, const VarMap &vars) override                                                       \
         {                                                                                                                                  \
         return run_dilithium_kat_test("Dilithium_" #mode "_" #algo, vars,                                                                  \
            Botan::DilithiumDimension::Dilithium##mode, Botan::DilithiumFlavor::algo, name);                                                \
         }                                                                                                                                  \
   };                                                                                                                                       \
   BOTAN_REGISTER_TEST("dilithium", "dilithium_kat_" #mode "_" #algo, DILITHIUM##mode##algo)
   ...
   REGISTER_DILITHIUM_KAT_TEST(4x4, AesDeterministic);
   REGISTER_DILITHIUM_KAT_TEST(6x5, AesDeterministic);
   REGISTER_DILITHIUM_KAT_TEST(8x7, AesDeterministic);
   REGISTER_DILITHIUM_KAT_TEST(4x4, AesRandomized);
   REGISTER_DILITHIUM_KAT_TEST(6x5, AesRandomized);
   REGISTER_DILITHIUM_KAT_TEST(8x7, AesRandomized);
#endif

class DilithiumRoundtripTests final : public Test
   {
   public:
      static Test::Result run_roundtrip(const char* test_name, Botan::DilithiumDimension dimension,
                                        Botan::DilithiumFlavor algo)
         {
         Test::Result result(test_name);

         auto sign = [](const auto& private_key, const auto& msg)
            {
            auto signer = Botan::PK_Signer(private_key, Test::rng(), "");
         ...
         Botan::Dilithium_PrivateKey priv_key(Test::rng(), dimension, algo);
         ...
         for(const auto encoding : encodings)
            ...
            Botan::Dilithium_PrivateKey priv_key_decoded(priv_key_encoded, dimension, algo, encoding);
            Botan::Dilithium_PublicKey pub_key_decoded(pub_key_encoded, dimension, algo, encoding);
   ...
   std::vector<Test::Result> run() override
      ...
      results.push_back(run_roundtrip("Dilithium_4x4_Common",
                                      Botan::DilithiumDimension::Dilithium4x4,
                                      Botan::DilithiumFlavor::Deterministic));
      results.push_back(run_roundtrip("Dilithium_6x5_Common",
                                      Botan::DilithiumDimension::Dilithium6x5,
                                      Botan::DilithiumFlavor::Deterministic));
      results.push_back(run_roundtrip("Dilithium_8x7_Common",
                                      Botan::DilithiumDimension::Dilithium8x7,
                                      Botan::DilithiumFlavor::Deterministic));
      results.push_back(run_roundtrip("Dilithium_4x4_Randomized",
                                      Botan::DilithiumDimension::Dilithium4x4,
                                      Botan::DilithiumFlavor::Randomized));
      results.push_back(run_roundtrip("Dilithium_6x5_Randomized",
                                      Botan::DilithiumDimension::Dilithium6x5,
                                      Botan::DilithiumFlavor::Randomized));
      results.push_back(run_roundtrip("Dilithium_8x7_Randomized",
                                      Botan::DilithiumDimension::Dilithium8x7,
                                      Botan::DilithiumFlavor::Randomized));
#endif
      ...
      results.push_back(run_roundtrip("Dilithium_4x4_AES",
                                      Botan::DilithiumDimension::Dilithium4x4,
                                      Botan::DilithiumFlavor::AesDeterministic));
      ...
      results.push_back(run_roundtrip("Dilithium_8x7_AES",
                                      Botan::DilithiumDimension::Dilithium8x7,
                                      Botan::DilithiumFlavor::AesDeterministic));
      results.push_back(run_roundtrip("Dilithium_4x4_AESRandomized",
                                      Botan::DilithiumDimension::Dilithium4x4,
                                      Botan::DilithiumFlavor::AesRandomized));
      results.push_back(run_roundtrip("Dilithium_6x5_AESRandomized",
                                      Botan::DilithiumDimension::Dilithium6x5,
                                      Botan::DilithiumFlavor::AesRandomized));
      results.push_back(run_roundtrip("Dilithium_8x7_AESRandomized",
                                      Botan::DilithiumDimension::Dilithium8x7,
                                      Botan::DilithiumFlavor::AesRandomized));
#endif

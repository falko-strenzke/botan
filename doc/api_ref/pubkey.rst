...
.. cpp:class:: PK_Encryption_Options
   ...
   .. cpp:function:: PK_Encryption_Options with_provider(std::string_view provider)
      ...
      only their own provider name.

      The hardware providers support ``OAEP`` and ``PKCS1v15``. PKCS#11 also
      supports ``Raw``. The TPM2 provider does not currently support an OAEP
      label, or an MGF1 hash differing from the OAEP hash.
...
.. cpp:class:: PK_Signature_Options
   ...
   .. cpp:function:: PK_Signature_Options with_provider(std::string_view provider)
      ...
      Request a specific implementation. This is rarely needed.
